...
public class ClusterManager {
     ...
     * @return                  Either a list of failed operations if an exception occurs during deletion, or the cluster ID if successful.
     */
    public Either<FailedOperation, String> createCluster(
        ...
        try {
            ...
            logger.log(Level.INFO, "Creating new cluster...");
            ...
            logger.log(
                    Level.INFO,
                    "View the cluster at " + workspaceClient.config().getHost()
                            + "#setting/clusters/"
                            + c.getClusterId()
                            + "/configuration\n");
     ...
     * @return                Either a Boolean indicating success or a list of failed operations if an exception occurs during cluster start.
     */
    public Either<FailedOperation, Boolean> startCluster(WorkspaceClient workspaceClient, String clusterID) {

        logger.log(Level.INFO, "Starting the cluster...");
        try {
            ClusterDetails c = workspaceClient.clusters().get(clusterID);
            if (c.getState() == State.RUNNING) {
                logger.log(
                        Level.INFO,
                        "Cluster is running at " + workspaceClient.config().getHost()
                                + "#setting/clusters/"
                                + clusterID
                                + "/configuration\n");
            ...
            logger.log(Level.INFO, "Error starting the cluster with ID " + clusterID);
            return left(new FailedOperation(Collections.singletonList(new Problem(e.getMessage()))));
        } catch (Exception e) {
            logger.log(Level.SEVERE, e.getMessage());
            return left(new FailedOperation(Collections.singletonList(new Problem(e.getMessage()))));
    ...
    public Either<FailedOperation, Void> stopCluster(WorkspaceClient workspaceClient, String clusterID) {
        logger.log(Level.INFO, "Stopping the cluster...");
        try {
            workspaceClient.clusters().delete(clusterID).get();
            logger.log(Level.INFO, "Cluster stopped");
            return right(null);
        } catch (Exception e) {
            logger.log(Level.SEVERE, e.getMessage());
            return left(new FailedOperation(Collections.singletonList(new Problem(e.getMessage()))));
    ...
    public Either<FailedOperation, Void> deleteCluster(WorkspaceClient workspaceClient, String clusterID) {
        ...
        try {
            ...
            logger.log(Level.INFO, "Cluster deleted");
